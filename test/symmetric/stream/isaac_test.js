...
module.exports = testCase({
  ...
  "Test ISAAC Vectors":function(test) {
    ...
    var pts = [];
    pts[0] = zeroedData(cts[0].length);
    pts[1] = zeroedData(cts[1].length);    
    pts[2] = new Array(256 * 4);
    for(var i = 0; i != pts[2].length; i++) {
      pts[2][i] = ((i % 4 == 0 || i % 4 == 1) ? 0xff : 0x00);
    }
    pts[3] = new Array(256 * 4);
    for(var i = 0; i != pts[3].length; i++) {
