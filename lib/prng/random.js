...
Random.prototype.seed = function(x) {
  ...
  this.pool[pptr++] ^= x & 255;
  this.pool[pptr++] ^= (x >> 8) & 255;
  this.pool[pptr++] ^= (x >> 16) & 255;
  this.pool[pptr++] ^= (x >> 24) & 255;
...
Random.prototype.nextBytes = function(length) {
  var vector = [];  
  while(length--) {
