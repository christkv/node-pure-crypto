...
// Basic stuff
MD5.prototype.getDigestSize = function() {
  return DIGEST_LENGTH;
}

MD5.prototype.getByteLength = function() {
  return BYTE_LENGTH;
}

MD5.prototype.getAlgorithmName = function() {
  return "MD5";
}
...
//
// Common to all digests
MD5.prototype.doFinal = function(output, index) {
  this.finish();
  index = index == null ? index : 0;

  // put final hash in output array
  util.inPlaceEncodeUInt32R(this.H1, output, index + 0);
  util.inPlaceEncodeUInt32R(this.H2, output, index + 4);
  util.inPlaceEncodeUInt32R(this.H3, output, index + 8);
  util.inPlaceEncodeUInt32R(this.H4, output, index + 12);  
  this.reset();
