...
Whirlpool.prototype.getDigestSize = function() {
  return DIGEST_LENGTH_BYTES;
}

Whirlpool.prototype.getAlgorithmName = function() {
  return "WHIRLPOOL";
}
...
//
// Common to all digests
Whirlpool.prototype.update = function(src, inOff, len) {
...
//
// Common to all digests
Whirlpool.prototype.doFinal = function(output, index) {
  this.finish();
  index = index == null ? 0 : index;
  // Encode the result
  for(var i = 0; i < 8; i++) {
    util.inPlaceEncodeUInt32(this._hash[i].getHighBits(), output, index + (i * 8));    
    util.inPlaceEncodeUInt32(this._hash[i].getLowBits(), output, index + (i * 8) + 4);
  }
