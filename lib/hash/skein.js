...
Configuration.prototype.generateConfiguration = function(initalState) {  
  var cipher = new TreeFish();
  cipher.init(true, this.stateSize);
...
var Skein = exports.Skein = function() {
}

Skein.prototype.init = function(stateSize, outputSize, treeInfo, key) {
  stateSize = stateSize == null ? 256 : stateSize;
  outputSize = outputSize == null ? 256 : outputSize;
  ...
  if(key != null && key.length > 0) { /* is there a key? */
    ...
    this.update(key, 0, key.length); /* hash the key */
...
var setup = function(self, stateSize, outputSize) {
  ...
  // the state size
  self.cipher = new TreeFish();
  self.cipher.init(true, stateSize);
...
Skein.prototype.getDigestSize = function() {
  return this.outputBytes;
}

Skein.prototype.getByteLength = function() {
  return this.cipherStateBytes;
}

Skein.prototype.getAlgorithmName = function() {
  return "SKEIN";
}
...
//
// Update by bits
Skein.prototype.updateBits = function(src, inOff, len) {
  ...
  if ((len & 0x7) == 0) {
    this.update(src, inOff, len >>> 3);
    return;
  }

  // Fill in bytes in buffer, add one for partial byte
  this.update(src, inOff, (len >>> 3) + 1);
...
Skein.prototype.update = function(src, inOff, len) {
  if(!Array.isArray(src)) src = util.binaryStringToArray(src);
  var bytesDone = 0;
  var len = len == null ? src.length : len;
  var start = inOff == null ? 0 : inOff;
...
//
// Common to all digests
Skein.prototype.doFinal = function(output, index) {
