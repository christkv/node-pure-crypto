...
CAST256.prototype.encrypt = function(src, index) {
  ...
  // return this.cryptBlock(src.slice(index, index + BlockSize), ENCRYPT);
  return this.cryptBlock(src, ENCRYPT, index);
}

// block must have length of BLOCKSIZE
// returns: crypted block
CAST256.prototype.cryptBlock = function(block, mode, index) {
  if(block.length < BlockSize) throw new Error("Wrong blocksize! Had to be " + BlockSize + " bytes!");
  
  // Encode the blocks
  var a = new Array(4);
  for(var i = 0; i < 4; i++) {
    a[i] = Long.fromNumber(util.decodeUInt32(block, index + (i * 4))).getLowBitsUnsigned();
  }
  ...
  util.inPlaceEncodeUInt32(a[0], block, index);
  util.inPlaceEncodeUInt32(a[1], block, index + 4);
  util.inPlaceEncodeUInt32(a[2], block, index + 8);
  util.inPlaceEncodeUInt32(a[3], block, index + 12);
  return block;
}
...
CAST256.prototype.decrypt = function(src, index) {
  index = index == null ? 0 : index;
  return this.cryptBlock(src, DECRYPT, index);
}
