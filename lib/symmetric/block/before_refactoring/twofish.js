...
var Twofish = exports.Twofish = function(keyBytes, rounds) {
...
Twofish.prototype.getBlockSize = function() {
  return BlockSize;
}

Twofish.prototype.encrypt = function(src, index) {
  index = index == null ? 0 : index;
  return this.encryptBlock(src, index);
}

Twofish.prototype.encryptBlock = function(block, srcIndex) {
  // Decode the integers
  ...
  a = Long.fromNumber(a ^ this.m_k[0]).getLowBitsUnsigned();
  b = Long.fromNumber(b ^ this.m_k[1]).getLowBitsUnsigned();
  c = Long.fromNumber(c ^ this.m_k[2]).getLowBitsUnsigned();
  d = Long.fromNumber(d ^ this.m_k[3]).getLowBitsUnsigned();

  // Point to an index 8 words forward
  var index = 8;
  // Contains the values used in the encryption
  var values = [this.m_k, [a, b, c, d], this.m_s];
  // Encrypt
  ...
  c = Long.fromNumber(values[1][2] ^ this.m_k[4]).getLowBitsUnsigned();
  d = Long.fromNumber(values[1][3] ^ this.m_k[5]).getLowBitsUnsigned();
  a = Long.fromNumber(values[1][0] ^ this.m_k[6]).getLowBitsUnsigned();
  b = Long.fromNumber(values[1][1] ^ this.m_k[7]).getLowBitsUnsigned();
...
Twofish.prototype.decrypt = function(src, index) {
  index = index == null ? 0 : index;
  return this.decryptBlock(src, index);
}

Twofish.prototype.decryptBlock = function(block, srcIndex) {
  // Decode the integers
  ...
  c = Long.fromNumber(c ^ this.m_k[4]).getLowBitsUnsigned();
  d = Long.fromNumber(d ^ this.m_k[5]).getLowBitsUnsigned();
  a = Long.fromNumber(a ^ this.m_k[6]).getLowBitsUnsigned();
  b = Long.fromNumber(b ^ this.m_k[7]).getLowBitsUnsigned();
  ...
  a = Long.fromNumber(values[1][0] ^ this.m_k[0]).getLowBitsUnsigned();
  b = Long.fromNumber(values[1][1] ^ this.m_k[1]).getLowBitsUnsigned();
  c = Long.fromNumber(values[1][2] ^ this.m_k[2]).getLowBitsUnsigned();
  d = Long.fromNumber(values[1][3] ^ this.m_k[3]).getLowBitsUnsigned();
