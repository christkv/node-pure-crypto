...
RC5.prototype.encrypt = function(src, index) {
  ...
  // Ensure input source is the size of a block
  if((src.length - index) < BlockSize) throw "input block must be " + BlockSize + " bytes long";
  return this.encryptBlock(src, index);
}

RC5.prototype.encryptBlock = function(input, index) {
...
RC5.prototype.decryptBlock = function(input, index) {
  // debug("input :: " + input.slice(index, index + BlockSize))
  ...
  // debug("output :: " + input.slice(index, index + BlockSize))
  return input;
}
