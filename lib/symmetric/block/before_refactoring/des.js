...
DESKey.prototype.init = function(encrypting, key) {
  this.key = key;
  this.encKey = this.generateWorkingKey(true, key, 0);
  this.decKey = this.generateWorkingKey(false, key, 0);
}
...
DESKey.prototype.encrypt = function(block, index) {
  ...
  return this.desFunc(this.encKey, block, index);
}

DESKey.prototype.decrypt = function(block, index) {
  index = index == null ? 0 : index;
  return this.desFunc(this.decKey, block, index);
}

DESKey.prototype.processBlock = function(src, index, encrypting) {
  if(encrypting) {
    return this.desFunc(this.encKey, src, index);
  } else {
    return this.desFunc(this.decKey, src, index);
  }
} 

DESKey.prototype.reset = function() {  
}
