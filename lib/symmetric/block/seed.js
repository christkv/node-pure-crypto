...
var Seed = exports.Seed = function(key) {
  this.encryptionKey = createKey(key);
  this.deCryptionKey = createKey(key, true);
}
...
Seed.prototype.getBlockSize = function() {
  return BlockSize;
}

Seed.prototype.encrypt = function(src, index) {
  index = index == null ? 0 : index;
  return this.cryptBlock(src.slice(index, index + BlockSize), this.encryptionKey);
}

Seed.prototype.cryptBlock = function(block, key) {
  var a0 = util.decodeUInt32(block, 0);
  var a1 = util.decodeUInt32(block, 4);
  var b0 = util.decodeUInt32(block, 8);
  var b1 = util.decodeUInt32(block, 12);
  var t0 = 0;
  var t1 = 0;
  ...
  return util.encodeUInt32(b0)
    .concat(util.encodeUInt32(b1))
    .concat(util.encodeUInt32(a0))
    .concat(util.encodeUInt32(a1));  
}

Seed.prototype.decrypt = function(src, index) {
  index = index == null ? 0 : index;
  return this.cryptBlock(src.slice(index, index + BlockSize), this.deCryptionKey);
}
