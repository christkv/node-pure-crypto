...
// Process a block
Camellia.prototype.processBlock = function(input, inOff, out, outOff) {
  inOff = inOff == null ? 0 : inOff;
  outOff = outOff == null ? 0 : outOff;  
  // Process the block
  return processBlock(this, input, inOff, out, outOff);
}

var processBlock = function(self, input, index, out, outOff) {
  index = index == null ? 0 : index;

  if(self._keyIs128) {
    return processBlock128(input, index, out, outOff, self.subKey, self.kW, self.kE, self.state);
  } else {
    return processBlock192or256(input, index, out, outOff, self.subKey, self.kW, self.kE, self.state);
  }
}

var processBlock192or256 = function(input, index, out, outOff, subkey, kw, ke, state) {
  for (var i = 0; i < 4; i++) {
  ...
  util.inPlaceEncodeUInt32(state[2], out, outOff);
  util.inPlaceEncodeUInt32(state[3], out, outOff + 4);
  util.inPlaceEncodeUInt32(state[0], out, outOff + 8);
  util.inPlaceEncodeUInt32(state[1], out, outOff + 12);
  return BlockSize;
}


var processBlock128 = function(input, index, out, outOff, subkey, kw, ke, state) {
  for (var i = 0; i < 4; i++) {
    state[i] = util.decodeUInt32(input, index + (i * 4));
    state[i] ^= kw[i];
  }
