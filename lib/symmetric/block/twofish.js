...
var Twofish = exports.Twofish = function() {
}

Twofish.prototype.init = function(forEncryption, keyBytes) {
  this.forEncryption = forEncryption;
  var len = (keyBytes.length <= 16 ? 2 : (keyBytes.length <= 24 ? 3 : 4));
  var key = new Array(len * 2);
  var svec = new Array(len * 2);
  var m_k = this.m_k = new Array(40);
  var m_s = this.m_s = new Array(4 * 256);
  
  for(var i = 0; i < (len * 2); i++) {
    key[i] = Long.fromNumber(util.decodeUInt32(keyBytes.slice((i*4), (i*4 + 4)).reverse(), 0)).getLowBitsUnsigned();
  }
  
  for(var i = 0; i < 40; i += 2) {
    var a = Long.fromNumber(h(i, key, len, 0)).getLowBitsUnsigned();
    var b = Long.fromNumber(util.rotl(Long.fromNumber(h(i+1, key, len, 1)).getLowBitsUnsigned(), 8)).getLowBitsUnsigned();
    m_k[i] = Long.fromNumber(a + b).getLowBitsUnsigned();
    m_k[i + 1] = Long.fromNumber(util.rotl(a + (2 * b), 9)).getLowBitsUnsigned();
  }
  
  for(var i = 0; i < len; i++) {
    svec[2 * (len - i - 1)] = Long.fromNumber(reedSolomon(key[(2 * i) + 1], key[2 * i])).getLowBitsUnsigned();    
  }
  
  for(var i = 0; i < 256; i++) {
    var t = Long.fromNumber(h0(i, svec, len, 0)).getLowBitsUnsigned();
    // Unpack the bytes
    var bytes = util.encodeUInt32(t).reverse();
    
    m_s[0 * 256 + i] = mds[0][bytes[0]];
    m_s[1 * 256 + i] = mds[1][bytes[1]];
    m_s[2 * 256 + i] = mds[2][bytes[2]];
    m_s[3 * 256 + i] = mds[3][bytes[3]];
  }  
}
...
var mod = function(c) {
  var modulus = 0x14d;
  var c2 = Long.fromNumber((c << 1) ^ ((c & 0x80) ? modulus : 0)).getLowBitsUnsigned();
  var c1 = Long.fromNumber(c2 ^ (c >> 1) ^ ((c & 1) ? (modulus >>> 1) : 0)).getLowBitsUnsigned();
...
var reedSolomon = function(high, low) {
  for(var i = 0; i < 8; i++) {
    high = Long.fromNumber(mod(high >>> 24) ^ (high << 8) ^ (low >>> 24)).getLowBitsUnsigned();
    low = Long.fromNumber(low << 8).getLowBitsUnsigned();
  }  
  return high;
}

var h = function(x, key, kLen, index) {
  x = h0(x, key, kLen, index);
  // Unpack the integer to bytes for lookup
  var bytes = util.encodeUInt32(x).reverse();
  // Return the value from the tables
  return Long.fromNumber(mds[0][bytes[0]] ^ mds[1][bytes[1]] ^ mds[2][bytes[2]] ^ mds[3][bytes[3]]).getLowBitsUnsigned();
}

var h0 = function(x, key, kLen, index) {
  x = Long.fromNumber(x | (x << 8) | (x << 16) | (x << 24)).getLowBitsUnsigned();
  switch(kLen) {
    case 4:
      x = Long.fromNumber(Q(1, 0, 0, 1, x) ^ key[6 + index]).getLowBitsUnsigned();
    case 3:
      x = Long.fromNumber(Q(1, 1, 0, 0, x) ^ key[4 + index]).getLowBitsUnsigned();    
    case 2:
      x = Long.fromNumber(Q(0, 1, 0, 1, x) ^ key[2 + index]).getLowBitsUnsigned();
    default:
      x = Long.fromNumber(Q(0, 0, 1, 1, x) ^ key[0 + index]).getLowBitsUnsigned();    
  }  
  return x;
}

var Q = function(a, b, c, d, t) {
  // Unpack the integer to bytes for lookup
  var bytes = util.encodeUInt32(t).reverse();
  return q[a][bytes[0]] ^ (q[b][bytes[1]] << 8) ^ (q[c][bytes[2]] << 16) ^ (q[d][bytes[3]] << 24);
}

// Block size of cipher
Twofish.prototype.getBlockSize = function() { return BlockSize; }
// Algorithm Name
Twofish.prototype.getAlgorithmName = function() { return "TWOFISH"; }
// Reset cipher
Twofish.prototype.reset = function() {}

// Process a block
Twofish.prototype.processBlock = function(input, inOff, out, outOff) {
...
var encryptBlock = function(self, block, srcIndex, out, outOff) {
  ...
  a = Long.fromNumber(a ^ self.m_k[0]).getLowBitsUnsigned();
  b = Long.fromNumber(b ^ self.m_k[1]).getLowBitsUnsigned();
  c = Long.fromNumber(c ^ self.m_k[2]).getLowBitsUnsigned();
  d = Long.fromNumber(d ^ self.m_k[3]).getLowBitsUnsigned();

  // Point to an index 8 words forward
  var index = 8;
  // Contains the values used in the encryption
  var values = [self.m_k, [a, b, c, d], self.m_s];
  // Encrypt
  values = encCycle(0, values, index);
  values = encCycle(1, values, index);
  values = encCycle(2, values, index);
  values = encCycle(3, values, index);
  values = encCycle(4, values, index);
  values = encCycle(5, values, index);
  values = encCycle(6, values, index);
  values = encCycle(7, values, index);

  c = Long.fromNumber(values[1][2] ^ self.m_k[4]).getLowBitsUnsigned();
  d = Long.fromNumber(values[1][3] ^ self.m_k[5]).getLowBitsUnsigned();
  a = Long.fromNumber(values[1][0] ^ self.m_k[6]).getLowBitsUnsigned();
  b = Long.fromNumber(values[1][1] ^ self.m_k[7]).getLowBitsUnsigned();

  // Return the encrypted bytes
  util.inPlaceEncodeUInt32R(c, out, outOff);
  util.inPlaceEncodeUInt32R(d, out, outOff + 4);
  util.inPlaceEncodeUInt32R(a, out, outOff + 8);
  util.inPlaceEncodeUInt32R(b, out, outOff + 12);
  return BlockSize;
}

var g1 = function(m_s, x) {
  var bytes = util.encodeUInt32(x).reverse();
  return Long.fromNumber(m_s[(0*256) + bytes[0]] ^ m_s[(1*256) + bytes[1]] ^ m_s[(2*256) + bytes[2]] ^ m_s[(3*256) + bytes[3]]).getLowBitsUnsigned();
}

var g2 = function(m_s, x) {
  var bytes = util.encodeUInt32(x).reverse();
  return Long.fromNumber(m_s[(0*256) + bytes[3]] ^ m_s[(1*256) + bytes[0]] ^ m_s[(2*256) + bytes[1]] ^ m_s[(3*256) + bytes[2]]).getLowBitsUnsigned();
}

var encRound = function(n, values, index) {
  var x = g1(values[2], values[1][0]); // unpack a
  var y = g2(values[2], values[1][1]); // unpack b
  x = Long.fromNumber(x + y).getLowBitsUnsigned();
  y = Long.fromNumber(y + x + values[0][2 * (n) + index + 1]).getLowBitsUnsigned();
  values[1][2] = Long.fromNumber(values[1][2] ^ x + values[0][(2 * n) + index]).getLowBitsUnsigned();
  values[1][2] = Long.fromNumber(util.rotr(values[1][2], 1)).getLowBitsUnsigned();
  // Calculate d
  values[1][3] = Long.fromNumber(util.rotl(values[1][3], 1) ^ y).getLowBitsUnsigned();
  return values;
}

var encCycle = function(n, values, index) {
  values = encRound((2 * n), values, index);
  values = encRound((2 * n) + 1, [values[0], [values[1][2], values[1][3], values[1][0], values[1][1]], values[2]], index);
  return [values[0], [values[1][2], values[1][3], values[1][0], values[1][1]], values[2]];
}

var decCycle = function(n, values, index) {
  values = decRound(2 * (n) + 1, [values[0], [values[1][2], values[1][3], values[1][0], values[1][1]], values[2]], index);  
  values = [values[0], [values[1][2], values[1][3], values[1][0], values[1][1]], values[2]];
  values = decRound((2 * n), [values[0], [values[1][0], values[1][1], values[1][2], values[1][3]], values[2]], index);
  return values;  
}
...
var decryptBlock = function(self, block, srcIndex, out, outOff) {
  // Decode the integers
  var c = Long.fromNumber(util.decodeUInt32R(block, srcIndex)).getLowBitsUnsigned();
  var d = Long.fromNumber(util.decodeUInt32R(block, srcIndex + 4)).getLowBitsUnsigned();
  var a = Long.fromNumber(util.decodeUInt32R(block, srcIndex + 8)).getLowBitsUnsigned();
  var b = Long.fromNumber(util.decodeUInt32R(block, srcIndex + 12)).getLowBitsUnsigned();

  c = Long.fromNumber(c ^ self.m_k[4]).getLowBitsUnsigned();
  d = Long.fromNumber(d ^ self.m_k[5]).getLowBitsUnsigned();
  a = Long.fromNumber(a ^ self.m_k[6]).getLowBitsUnsigned();
  b = Long.fromNumber(b ^ self.m_k[7]).getLowBitsUnsigned();
  ...
  var values = [self.m_k, [a, b, c, d], self.m_s];
  // Encrypt
  values = decCycle(7, values, index);
  values = decCycle(6, values, index);
  values = decCycle(5, values, index);
  values = decCycle(4, values, index);
  values = decCycle(3, values, index);
  values = decCycle(2, values, index);
  values = decCycle(1, values, index);
  values = decCycle(0, values, index);

  a = Long.fromNumber(values[1][0] ^ self.m_k[0]).getLowBitsUnsigned();
  b = Long.fromNumber(values[1][1] ^ self.m_k[1]).getLowBitsUnsigned();
  c = Long.fromNumber(values[1][2] ^ self.m_k[2]).getLowBitsUnsigned();
  d = Long.fromNumber(values[1][3] ^ self.m_k[3]).getLowBitsUnsigned();
