...
var AESKey = exports.AESKey = function(key) {
  // Private state variables
  this.ROUNDS = 0;
  // Generate encryption key
  this.encryptionWorkingKey = this.generateWorkingKey(key, true);
  // Generate decryption key
  this.decryptionWorkingKey = this.generateWorkingKey(key, false);
}

AESKey.prototype.generateWorkingKey = function(key, forEncryption) {
  ...
  var W = new Array(this.ROUNDS + 1);
  for(var i = 0; i < W.length; i++) W[i] = new Array(4);
...
AESKey.prototype.getBlockSize = function() { return BLOCK_SIZE; }

// decrypt one 128 bit block
AESKey.prototype.decrypt = function(src, index) {
  index = index == null ? 0 : index;  
  return this.decryptBlock(src, index, this.decryptionWorkingKey);
}

AESKey.prototype.decryptBlock = function(input, index, KW) {
  var C0 = util.decodeUInt32R(input, index);
  var C1 = util.decodeUInt32R(input, index + 4);
  var C2 = util.decodeUInt32R(input, index + 8);
  var C3 = util.decodeUInt32R(input, index + 12);
  
  var r = 0, r0 = 0, r1 = 0, r2 = 0, r3 = 0;
  ...
  util.inPlaceEncodeUInt32R(C0, input, index + 0);
  util.inPlaceEncodeUInt32R(C1, input, index + 4);
  util.inPlaceEncodeUInt32R(C2, input, index + 8);
  util.inPlaceEncodeUInt32R(C3, input, index + 12);  
  return input;
}


// encrypt one 128 bit block
AESKey.prototype.encrypt = function(src, index) {
  index = index == null ? 0 : index;  
  return this.encryptBlock(src, index, this.encryptionWorkingKey);
}

AESKey.prototype.encryptBlock = function(input, index, KW) {
  var C0 = util.decodeUInt32R(input, index);
  var C1 = util.decodeUInt32R(input, index + 4);
  var C2 = util.decodeUInt32R(input, index + 8);
  var C3 = util.decodeUInt32R(input, index + 12);
  ...
  util.inPlaceEncodeUInt32R(C0, input, index);
  util.inPlaceEncodeUInt32R(C1, input, index + 4);
  util.inPlaceEncodeUInt32R(C2, input, index + 8);
  util.inPlaceEncodeUInt32R(C3, input, index + 12);  
  return input;
