...
// Process a block
TripleDESKey.prototype.processBlock = function(input, inOff, inLen) {
  inOff = inOff == null ? 0 : inOff;
  
  if(this.forEncryption) {
    var encrypted = desFunc(this.key, input, inOff);
    encrypted = desFunc(this.key2, encrypted, inOff);
    desFunc(this.key3, encrypted, inOff);    
  } else {
    var decrypted = desFunc(this.key3, input, inOff);
    decrypted = desFunc(this.key2, decrypted, inOff);
    desFunc(this.key, decrypted, inOff);    
  }
}
 ...
 * the DES engine.
 */
var desFunc = function(wKey, inp, inOff) {
	var work = 0;
	var right = 0;
	var left = 0;
	...
	inp[inOff + 0] = ((right >>> 24) & 0xff);
	inp[inOff + 1] = ((right >>> 16) & 0xff);
	inp[inOff + 2] = ((right >>> 8) & 0xff);
	inp[inOff + 3] = (right & 0xff);
	inp[inOff + 4] = ((left >>> 24) & 0xff);
	inp[inOff + 5] = ((left >>> 16) & 0xff);
	inp[inOff + 6] = ((left >>> 8) & 0xff);
	inp[inOff + 7] = (left & 0xff);
	return inp;
}
