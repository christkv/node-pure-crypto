...
var BlowFish = exports.BlowFish = function() {
}

BlowFish.prototype.init = function(forEncryption, key) {
...
BlowFish.prototype.getBlockSize = function() { return BLOCK_SIZE; }
// Algorithm Name
BlowFish.prototype.getAlgorithmName = function() { return "BLOWFISH"; }
// Reset cipher
BlowFish.prototype.reset = function() {}

// Process a block
BlowFish.prototype.processBlock = function(input, inOff, out, outOff) {
...
var setKey = function(self, key) {
  ...
  for(var i = 0; i < P_SZ; i++) {
	 ...
	 */
	 processTable(self, 0, 0, self.P);
	 processTable(self, self.P[P_SZ - 2], self.P[P_SZ - 1], self.S0);
	 processTable(self, self.S0[SBOX_SK - 2], self.S0[SBOX_SK - 1], self.S1);
	 processTable(self, self.S1[SBOX_SK - 2], self.S1[SBOX_SK - 1], self.S2);
	 processTable(self, self.S2[SBOX_SK - 2], self.S2[SBOX_SK - 1], self.S3);
}

var F = function(self, x) {
	return (((self.S0[(x >>> 24)] + self.S1[(x >>> 16) & 0xff]) ^ self.S2[(x >>> 8) & 0xff]) + self.S3[x & 0xff]);  
}

/**
 * apply the encryption cycle to each value pair in the table.
 */
var processTable = function(self, xl, xr, table) {
  ...
  for(var s = 0; s < size; s += 2) {
    ...
    for(var i = 1; i < ROUNDS; i += 2) {
      xr ^= F(self, xl) ^ self.P[i];
      xl ^= F(self, xr) ^ self.P[i + 1];
    }
...
var encryptBlock = function(self, src, index, out, outOff) {
  var xl = util.decodeUInt32(src, index);
  var xr = util.decodeUInt32(src, index + 4);
  ...
  xr ^= self.P[ROUNDS + 1];
  // Return the bytes for the two integers
  util.inPlaceEncodeUInt32(xr, out, outOff);
  util.inPlaceEncodeUInt32(xl, out, outOff + 4);
  return BLOCK_SIZE;
}
...
// ====================================
var ROUNDS = 16;
var BLOCK_SIZE = 8  // 64 bits
var SBOX_SK = 256;
var P_SZ = ROUNDS + 2;

var KP = [ 0x243F6A88, 0x85A308D3, 0x13198A2E, 0x03707344, 0xA4093822, 0x299F31D0,
	0x082EFA98, 0xEC4E6C89, 0x452821E6, 0x38D01377, 0xBE5466CF, 0x34E90C6C, 0xC0AC29B7, 0xC97C50DD, 0x3F84D5B5,
	0xB5470917, 0x9216D5D9, 0x8979FB1B ];

var KS0 = [ 0xD1310BA6, 0x98DFB5AC, 0x2FFD72DB, 0xD01ADFB7, 0xB8E1AFED, 0x6A267E96, 0xBA7C9045, 0xF12C7F99, 0x24A19947,
...
var KS1 = [ 0x4B7A70E9, 0xB5B32944, 0xDB75092E, 0xC4192623, 0xAD6EA6B0, 0x49A7DF7D, 0x9CEE60B8, 0x8FEDB266, 0xECAA8C71,
...
var KS2 = [ 0xE93D5A68, 0x948140F7, 0xF64C261C, 0x94692934, 0x411520F7, 0x7602D4F7, 0xBCF46B2E, 0xD4A20068, 0xD4082471,
...
var KS3 = [ 0x3A39CE37, 0xD3FAF5CF, 0xABC27737, 0x5AC52D1B, 0x5CB0679E, 0x4FA33742, 0xD3822740, 0x99BC9BBE, 0xD5118E9D,
