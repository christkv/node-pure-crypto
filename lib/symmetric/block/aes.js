...
AES.prototype.init = function(forEncryption, key) {
  // debug("============================================== AES")
  // debug("forEncryption :: " + forEncryption)
  // debug("key :: " + key)  
  this.forEncryption = forEncryption;
  this.sBox = _Sbox.slice(0);
  this.invSBox = _InvSbox.slice(0);
  this.xTime2Sbox = _Xtime2Sbox.slice(0);
  this.xTime3Sbox = _Xtime3Sbox.slice(0);
  this.xTime2 = _Xtime2.slice(0);
  this.xTime9 = _Xtime9.slice(0);
  this.xTimeB = _XtimeB.slice(0);
  this.xTimeD = _XtimeD.slice(0);
  this.xTimeE = _XtimeE.slice(0);
  this.rCon = _Rcon.slice(0);
  
  // Setup for key related values
  this.tmp = [];
  this.state = [];
  this.keyLength = key.length;
  // Ensure we have a copy of the key
...
var expandKey = function(self) {
	var Nk = self.key.length / 4;
	var key = self.key;
	var Sbox = self.sBox;
	var Rcon = self.rCon;
	self.Nr = Nk + 6;
	
	for(var idx = Nk; idx < Nb * (self.Nr + 1); idx++ ) {
...
AES.prototype.getBlockSize = function() { return BLOCK_SIZE; }
// Algorithm Name
AES.prototype.getAlgorithmName = function() { return "AES"; }
// Reset cipher
AES.prototype.reset = function() {}

// Process a block
AES.prototype.processBlock = function(input, inOff, out, outOff) {
  // debug("------------------------------------------- AES::processBlock")
  // debug(input)
...
var decryptBlock = function(self, data, index, out, outOff) {
  var block = data;
  var round = 0;
  if(index == null) index = 0;
  // Copy array
  self.state = block.slice(index, index + (Nb*4))
  // Start decryption
  addRoundKey(self, self.key, (self.Nr * Nb * 4)); 
  invShiftRows(self);

  for(var round = self.Nr; round--; ) {
   addRoundKey(self, self.key, round * Nb * 4);
   if (round) {
     invMixSubColumns(self);
   }
  }

  // Update data with state
  util.copy(out, outOff, self.state, 0, (Nb*4));
  return BLOCK_SIZE;
}
...
var invShiftRows = function(self) {
	var tmp = 0;
	var state = self.state;
	var InvSbox = self.invSBox;
...
var invMixSubColumns = function(self) {
  var tmp = self.tmp = [], i = 0;
	var XtimeE = self.xTimeE, XtimeB = self.xTimeB, XtimeD = self.xTimeD, Xtime9 = self.xTime9;
	var state = self.state;
	var InvSbox = self.invSBox;
...
var encryptBlock = function(self, data, index, out, outOff) {
  var block = data;
  var round = 0;
  if(index == null) index = 0;
  // Copy array
  self.state = block.slice(index, index + (Nb*4));
  // Round the key
  addRoundKey(self, self.key, 0);

  for(round = 1; round < self.Nr + 1; round++) {
    if(round < self.Nr) {
      mixSubColumns(self);
    } else {
      shiftRows(self);
...
var addRoundKey = function(self, key, offset) {
...
var mixSubColumns = function(self) {
  self.tmp = new Array(16);
  var tmp = self.tmp;
  var state = self.state;  
  var Xtime2Sbox = self.xTime2Sbox, Xtime3Sbox = self.xTime3Sbox;
  var Sbox = self.sBox;
...
var shiftRows = function(self) {
  var tmp = 0;
  var state = self.state, Sbox = self.sBox;
...
// AES only supports Nb=4, number of columns in the state & expanded key
var Nb = 4;
