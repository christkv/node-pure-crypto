...
CBCMode.prototype.updateDecrypt = function(src, binary) {
  ...
  if(src.length > this.blockSize) {
    ...
    return binary != null && binary ? this.decrypt(src, true) : util.arrayToBinaryString(this.decrypt(src, true));
  } else {
    // Less than the blocksize pad it
    return binary != null && binary ? this.decrypt(src) : util.arrayToBinaryString(this.decrypt(src));
  }
}

CBCMode.prototype.finalDecrypt = function(binary) {
  return binary != null && binary ? this.decrypt(this.lastBytes) : util.arrayToBinaryString(this.decrypt(this.lastBytes));
}
...
CBCMode.prototype.updateEncrypt = function(src, binary) {
  ...
  return binary != null && binary ? this.encrypt(src, true) : util.arrayToBinaryString(this.encrypt(src, true));
}
