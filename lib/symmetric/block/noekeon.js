...
Noekeon.prototype.encrypt = function(src, index) {
  ...
  if(src.length % BlockSize > 0) throw "input data must be multiple of " + BlockSize + " bytes";
  var numberOfBlocks = (src.length - index) / BlockSize;
  for(var i = index; i < numberOfBlocks; i++) {
    this.encryptBlock(src, i * BlockSize);
  }
  
  return src;
}

Noekeon.prototype.encryptBlock = function(input, index) {
  this.state[0] = util.decodeUInt32(input, index + 0);
  this.state[1] = util.decodeUInt32(input, index + 4);
  this.state[2] = util.decodeUInt32(input, index + 8);
  this.state[3] = util.decodeUInt32(input, index + 12);
  ...
  // Write out data
  util.inPlaceEncodeUInt32(this.state[0], input, index + 0);
  util.inPlaceEncodeUInt32(this.state[1], input, index + 4);
  util.inPlaceEncodeUInt32(this.state[2], input, index + 8);
  util.inPlaceEncodeUInt32(this.state[3], input, index + 12);  
  return input; 
}
...
Noekeon.prototype.decrypt = function(src, index) {
  ...
  var numberOfBlocks = (src.length - index) / BlockSize;
  for(var i = index; i < numberOfBlocks; i++) {
    this.decryptBlock(src, i * BlockSize);
  }
  
  return src;
}

Noekeon.prototype.decryptBlock = function(input, index) {
  this.state[0] = util.decodeUInt32(input, index + 0);
  this.state[1] = util.decodeUInt32(input, index + 4);
  this.state[2] = util.decodeUInt32(input, index + 8);
  this.state[3] = util.decodeUInt32(input, index + 12);
