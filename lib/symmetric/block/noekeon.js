...
// Process a block
Noekeon.prototype.processBlock = function(input, inOff, inLen) {
...
var encryptBlock = function(self, input, index) {
  ...
  // Write out data
  util.inPlaceEncodeUInt32(self.state[0], input, index + 0);
  util.inPlaceEncodeUInt32(self.state[1], input, index + 4);
  util.inPlaceEncodeUInt32(self.state[2], input, index + 8);
  util.inPlaceEncodeUInt32(self.state[3], input, index + 12);  
}
