...
// Cast6/Cast 256 Implementation in Javascript
var BlockSize = 16;
var ND = 0xFFFFFFFF;
var ENCRYPT = 0;
var DECRYPT = 1;

var CAST256 = exports.CAST256 = function(in_key) {
  // Ensure we are not modifying the original key
  in_key = in_key.slice(0);
  // Pad out key if we have less than 32 bytes (256)
  if(in_key.length < 32) {
    for(var i = in_key.length; i < 32; i++) {
      in_key.push(0);
    }
  }
  ...
  for(i = 0; i < 8; i ++) {
    a[i] = Long.fromNumber(util.decodeUInt32(in_key, (i * 4))).getLowBitsUnsigned();
  }

  // Create keys
  this.encryptKey = this.generateSubkeys(a, ENCRYPT);
  this.decryptKey = this.generateSubkeys(a, DECRYPT);
}

var Subkeys = function(km, kr) {
  this.km = km;
  this.kr = kr;
}

// Generates the subkeys and returns them.
// If "cryptmode" is ENCRYPT, then this function creates keys 
// to encrypt data with the function "crypt_block!", otherwise
// the created keys are to DECRYPT with the "crypt_block!" funtion.
// 
// returns: subkeys for calling crypt_block!
CAST256.prototype.generateSubkeys = function(key, cryptmode) {
  var cr = 19;
  var mr = 17;
  var cm = Long.fromNumber(0x5A827999);	  // 2**30 * sqrt(2)
  var mm = Long.fromNumber(0x6ED9EBA1);	  // 2**30 * sqrt(3)
  
  // Clone the key before modifying it
  key = key.slice(0);
  var sk = new Subkeys(new Array(12), new Array(12));
  ...
  // Reverse they key for decryption
  if(cryptmode == DECRYPT) {
    // Reverse content in the arrays (javascript does in place reversal)
    sk.km.reverse();
    sk.kr.reverse();
  }
  // Return the subkey
...
// 
// forward octave (only used to create the subkeys)
// changes: bitblock256
...
CAST256.prototype.keySchedule = function(inn) {
}

CAST256.prototype.getBlockSize = function() {
  return BlockSize;
}

CAST256.prototype.encrypt = function(src, index) {
  index = index == null ? 0 : index;
  return this.cryptBlock(src.slice(index, BlockSize), ENCRYPT);
}

// block must have length of BLOCKSIZE
// returns: crypted block
CAST256.prototype.cryptBlock = function(block, mode) {
  if(block.length != BlockSize) throw new Error("Wrong blocksize! Had to be " + BlockSize + " bytes!");
  
  // Encode the blocks
  var a = new Array(4);
  for(var i = 0; i < 4; i++) {
    a[i] = Long.fromNumber(util.decodeUInt32(block, (i * 4))).getLowBitsUnsigned();
  }

  // Finish Up encryption the block
  this.cryptBitBlock128(a, mode == ENCRYPT ? this.encryptKey : this.decryptKey);
  
  // Return the encrypted bytes
  return util.encodeUInt32(a[0])
    .concat(util.encodeUInt32(a[1]))
    .concat(util.encodeUInt32(a[2]))
    .concat(util.encodeUInt32(a[3]));
}

CAST256.prototype.cryptBitBlock128 = function(bitblock128, sk) {
  for(var i = 0; i < 6; i++) {
    this.forwardQuadround(bitblock128, sk, i);
  }
  
  for(var i = 6; i < 12; i++) {
    this.reverseQuadround(bitblock128, sk, i);    
  }
}

// 
// function Q, forward quad-round
// changes: bitblock128
//  
CAST256.prototype.forwardQuadround = function(bitblock128, sk, i) {  
  var c = f1(bitblock128[3], sk.kr[i][0], sk.km[i][0]);
  bitblock128[2] = Long.fromNumber(bitblock128[2]).xor(Long.fromNumber(c)).toNumber();

  var b = f2(bitblock128[2], sk.kr[i][1], sk.km[i][1]);
  bitblock128[1] = Long.fromNumber(bitblock128[1]).xor(Long.fromNumber(b)).toNumber();

  var a = f3(bitblock128[1], sk.kr[i][2], sk.km[i][2]);
  bitblock128[0] = Long.fromNumber(bitblock128[0]).xor(Long.fromNumber(a)).toNumber();

  var d = f1(bitblock128[0], sk.kr[i][3], sk.km[i][3]);
  bitblock128[3] = Long.fromNumber(bitblock128[3]).xor(Long.fromNumber(d)).toNumber();
}

// 
// function Q', reverse quad-round
// changes: bitblock128
//  
CAST256.prototype.reverseQuadround = function(bitblock128, sk, i) {
  var d = f1(bitblock128[0], sk.kr[i][3], sk.km[i][3]);
  bitblock128[3] = Long.fromNumber(bitblock128[3]).xor(Long.fromNumber(d)).toNumber();
...
CAST256.prototype.decrypt = function(src, index) {
  index = index == null ? 0 : index;
  return this.cryptBlock(src.slice(index, BlockSize), DECRYPT);
}

var iRnd = function(l_key, x, n) {
  x[3] ^= f1(x[0], l_key[n + 3], l_key[n + 7]);
  x[0] ^= f3(x[1], l_key[n + 2], l_key[n + 6]);
  x[1] ^= f2(x[2], l_key[n + 1], l_key[n + 5]);
  x[2] ^= f1(x[3], l_key[n], l_key[n + 4]);
}

// Round
var fRnd = function(l_key, x ,n) {
  x[2] ^= f1(x[3], l_key[n], l_key[n + 4]);
  x[1] ^= f2(x[2], l_key[n + 1], l_key[n + 5]);
  x[0] ^= f3(x[1], l_key[n + 2], l_key[n + 6]);
  x[3] ^= f1(x[0], l_key[n + 3], l_key[n + 7]);
...
// Key round
var kRnd = function(k, tr, tm) {
  k[6] ^= f1(k[7], tr[0], tm[0]);
  k[5] ^= f2(k[6], tr[1], tm[1]);
  k[4] ^= f3(k[5], tr[2], tm[2]);
  k[3] ^= f1(k[4], tr[3], tm[3]);
  k[2] ^= f2(k[3], tr[4], tm[4]);
  k[1] ^= f3(k[2], tr[5], tm[5]);
  k[0] ^= f1(k[1], tr[6], tm[6]);
  k[7] ^= f2(k[0], tr[7], tm[7]);
}

var rotl = function(x, n) {
  return TWO_PWR_32_DBL_ + ((x << n) | (x >>> (32 - n)));
}

var rotl2 = function(x, n) {
  return TWO_PWR_32_DBL_ + ((x << n) | (x >>> (32 - n)));
}

// F1 function
var f1 = function(d, kr, km) {
  var i  = rotl(km + d, kr);
  var ia = (i >>> 24) & 0xFF;
  var ib = (i >>> 16) & 0xFF;
  var ic = (i >>> 8)  & 0xFF;
  ...
  // Return
  return Long.fromNumber(s_box[0][ia])
    .xor(Long.fromNumber(s_box[1][ib]))
    .subtract(Long.fromNumber(s_box[2][ic]))
    .add(Long.fromNumber(s_box[3][id])).getLowBitsUnsigned();
}

var TWO_PWR_16_DBL_ = 1 << 16;
var TWO_PWR_32_DBL_ = TWO_PWR_16_DBL_ * TWO_PWR_16_DBL_;

var f2 = function(d, kr, km) {
  var r = Long.fromNumber(km).xor(Long.fromNumber(d));
  var r1 = r.shiftLeft(kr);
  var r2 = r.shiftRight(32 - kr);
  var i = r1.or(r2).getLowBitsUnsigned();
  ...
  // Return  
  return Long.fromNumber(s_box[0][ia])
    .subtract(Long.fromNumber(s_box[1][ib]))
    .add(Long.fromNumber(s_box[2][ic]))
    .xor(Long.fromNumber(s_box[3][id])).getLowBitsUnsigned();
}

var f3 = function(d, kr, km) {
  var r = (km - d)&ND;
  var i = r;
  ...
  // Return  
  return Long.fromNumber(s_box[0][ia])
    .add(Long.fromNumber(s_box[1][ib]))
    .xor(Long.fromNumber(s_box[2][ic]))
    .subtract(Long.fromNumber(s_box[3][id])).getLowBitsUnsigned();
}

// SBox for 
var s_box = new Array(4);
s_box[0] = new Array(
