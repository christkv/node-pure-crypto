...
XSalsa20.prototype.processBytes = function(src, index) {
  ...
  while(i < l) {
    ...
    if((i + 4) >= src.length) return;
    encodeStream(src, i + 4, x13);
    if((i + 8) >= src.length) return;
    encodeStream(src, i + 8, x10);
    if((i + 12) >= src.length) return;
    encodeStream(src, i + 12, x7);

    if((i + 16) >= src.length) return;
    encodeStream(src, i + 16, x4);
    if((i + 20) >= src.length) return;
    encodeStream(src, i + 20, x1);
    if((i + 24) >= src.length) return;
    encodeStream(src, i + 24, x14);
    if((i + 28) >= src.length) return;
    encodeStream(src, i + 28, x11);

    if((i + 32) >= src.length) return;
    encodeStream(src, i + 32, x8);
    if((i + 36) >= src.length) return;
    encodeStream(src, i + 36, x5);
    if((i + 40) >= src.length) return;
    encodeStream(src, i + 40, x2);
    if((i + 44) >= src.length) return;
    encodeStream(src, i + 44, x15);

    if((i + 48) >= src.length) return;
    encodeStream(src, i + 48, x12);
    if((i + 52) >= src.length) return;
    encodeStream(src, i + 52, x9);
    if((i + 56) >= src.length) return;
    encodeStream(src, i + 56, x6);
    if((i + 60) >= src.length) return;
