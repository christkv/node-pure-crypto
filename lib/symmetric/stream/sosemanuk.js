...
var Sosemanuk = exports.Sosemanuk = function(key, iv) {
  ...
  // Let's set up the iv :)
  a = Long.fromNumber(util.decodeUInt32(iv.slice(0, 4).reverse(), 0)).getLowBitsUnsigned();
  b = Long.fromNumber(util.decodeUInt32(iv.slice(4, 8).reverse(), 0)).getLowBitsUnsigned();
  c = Long.fromNumber(util.decodeUInt32(iv.slice(8, 12).reverse(), 0)).getLowBitsUnsigned();
  d = Long.fromNumber(util.decodeUInt32(iv.slice(12, 16).reverse(), 0)).getLowBitsUnsigned();
...
Sosemanuk.prototype.encrypt = function(src, index) {
  index = index == null ? 0 : index;
  return this.encryptStream(src);
}

Sosemanuk.prototype.updateEncrypt = function(src) {
...
Sosemanuk.prototype.encryptStream = function(src) {
  var self = this;  
...
Sosemanuk.prototype.decrypt = function(src, index) {
  index = index == null ? 0 : index;
  return this.encryptStream(src);
}

Sosemanuk.prototype.updateDecrypt = function(src) {
  var data = !Array.isArray(src) ? util.binaryStringToArray(src) : src.slice(0);
  return util.arrayToBinaryString(this.encryptStream(data));
}

Sosemanuk.prototype.finalDecrypt = function() {
  return "";  
}
