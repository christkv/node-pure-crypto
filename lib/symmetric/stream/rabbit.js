...
Rabbit.prototype.processBytes = function(input, inOff, len, out, outOff) {
  inOff = inOff == null ? 0 : inOff;
  outOff = outOff == null ? 0 : outOff;
  // Encrypt data
  _rabbit(this, input, inOff, len, out, outOff);
}

Rabbit.prototype.returnByte = function(value) {
  var data = [value];
  return _rabbit(this, data, 0, 1, data, 0)[0];
}

// Encryption/decryption scheme
var _rabbit = function(self, m, index, len, out, outOff) {
  for(var i = 0; i < len; i++, self.sPosition++) {
    // Get stream box
    var s = self.s;
    ...
    out[outOff + i] = m[index + i] ^ s[self.sPosition % 16];    
  }
